...
def get_recomendaciones(titulo, normalized_embeddings, indices, dataSet, top_n=10):
...
def main():
    ...
    ratings_test = pd.read_csv('MoviesandTV_test.csv',
                               names=['item_id', 'user_id', 'ratings'], nrows=N)
    ratings_train = pd.read_csv('MoviesandTV_train.csv',
                                names=['item_id', 'user_id', 'ratings'], nrows=N)
    ratings_val = pd.read_csv('MoviesandTV_val.csv',
                              names=['item_id', 'user_id', 'ratings'], nrows=N)
    dataSet = pd.read_csv('/home/jupyter-tfg2425multidomini-163ed/meta_Movies_and_TV.csv',
                          names=['id', 'main_category', 'title', 'price', 'description'], nrows=N)
